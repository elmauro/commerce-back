...
namespace MC.CommerceService.API.Controllers.v1
{
    ...
    public class ProductController : ControllerBase
    {
        ...
        /// <returns>The list of products if found; otherwise, a 404 status code.</returns>
        [HttpGet]
        [ProducesResponseType(typeof(IActionDataResponse<List<ProductView>>), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(ErrorResponse), StatusCodes.Status404NotFound)]
        public async Task<IActionResult> GetAllPaginated([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10)
        {
            return await _mediator.Send(new GetPaginatedProductsQuery(pageNumber, pageSize));
        }
    }
}
