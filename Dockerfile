...
RUN dotnet build MC.CommerceService.sln -c Release

# Package the compiled application into a folder for deployment
RUN dotnet publish MC.CommerceService.sln --no-restore -c Release --output /out/
...
ENTRYPOINT ["dotnet", "MC.CommerceService.API.dll"]
